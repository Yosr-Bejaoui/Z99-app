...
c = c.replace(/const renderTransactionsModal = \(\) => \([\s\S]*?<\/Modal>\n\s*\);\n/, '');

c = c.replace(/\<TouchableOpacity onPress=\{.*?setShowTransactions\(true\).*?\>[\s\S]*?<\/TouchableOpacity>/, '');

c = c.replace(/const \[showTransactions, setShowTransactions\] = useState\(false\);\n/, '');
c = c.replace(/\{renderTransactionsModal\(\)\}/, `
