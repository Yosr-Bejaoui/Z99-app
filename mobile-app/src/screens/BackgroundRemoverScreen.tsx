...
const BackgroundRemoverScreen: React.FC = () => {
  ...
  const fetchModel = async () => {
    try {
      ...
      const bgModel = response.results.find(
        (m: any) => m.model_id === 'wavespeed-ai/image-background-remover'
      );
      if (bgModel) {
        setModel(bgModel);
      }
  ...
  const processImage = async () => {
    ...
    if (!prompt.trim()) {
        Alert.alert('Error', 'Please enter a description for the new background.');
        return;
    }
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
         <GlassCard style={styles.card}>
            ...
            <Text style={styles.desc}>Remove the background from any image.</Text>
